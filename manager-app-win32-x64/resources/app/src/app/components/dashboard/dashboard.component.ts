...
export class DashboardComponent implements OnInit, ContentComponent {
  ...
  onComplete(event:any):void {
    if (this.cachedTaskName != undefined) {
      let task:Task = this.getTask(this.cachedTaskName);
      task.completed = true;
  ...
  onReject(event:any):void {
    if (this.cachedTaskName != undefined) {
      ...
      dialogRef.afterClosed().subscribe(result => {
        if(result != undefined) {
          let rejection:Rejection = {reason: result.reason, creationDate: result.creationDate};
          targetTask.rejections.push(rejection);
          this.fileService.emitChange(true);
        }
      })
    }
